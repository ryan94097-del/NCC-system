...
def multi_pass_verify(item, cert_index, platforms=None, max_detail=3, delay=0.8,
                      price_min=None, price_max=None, pool_key="", confirmed=0, need=0, attempts=0, on_status=None):
    """多輪搜尋：嘗試不同關鍵字直到找到結果。"""
    ...
    for kw in keywords:
        ...
        if "yahoo" in platforms:
            if on_status:
                on_status(pool_key, confirmed, need, attempts, item, store="Yahoo購物", kw=kw, url=f"https://tw.buy.yahoo.com/search/product?p={kw}")
            yc, _ = scrapers.search_yahoo(kw)
            cands.extend(yc)

        if "momo" in platforms:
            if on_status:
                on_status(pool_key, confirmed, need, attempts, item, store="MOMO購物", kw=kw, url=f"https://www.momoshop.com.tw/search/searchShop.jsp?keyword={kw}")
            mc, _ = scrapers.search_momo(kw, max_detail=max_detail, delay=delay)
            cands.extend(mc)

        if "ruten" in platforms:
            if on_status:
                on_status(pool_key, confirmed, need, attempts, item, store="露天拍賣", kw=kw, url=f"https://find.ruten.com.tw/s/?q={kw}")
...
def discover_other_rcb(year, want_cat, quota, platforms,
    ...
    while confirmed < quota and kw_idx < len(keywords):
        ...
        if "yahoo" in platforms:
            if on_status:
                on_status(pool_key, confirmed, quota, kw_idx, dummy_item, store="Yahoo購物", kw=kw, url=f"https://tw.buy.yahoo.com/search/product?p={kw}")
            yc, _ = scrapers.search_yahoo(kw)
            all_cands.extend(yc)
        if "momo" in platforms:
            if on_status:
                on_status(pool_key, confirmed, quota, kw_idx, dummy_item, store="MOMO購物", kw=kw, url=f"https://www.momoshop.com.tw/search/searchShop.jsp?keyword={kw}")
            mc, _ = scrapers.search_momo(kw, max_detail=max_detail, delay=delay)
            all_cands.extend(mc)
        if "ruten" in platforms:
            if on_status:
                on_status(pool_key, confirmed, quota, kw_idx, dummy_item, store="露天拍賣", kw=kw, url=f"https://find.ruten.com.tw/s/?q={kw}")
            rc, _ = scrapers.search_ruten(kw, max_items=5)
            all_cands.extend(rc)
    ...
    for pk in ["LPD_CCAN", "TTE_CCAN", "DOC_CCAN"]:
        ...
        for it in pool_items:
            ...
            res = multi_pass_verify(it, cert_index, platforms=platforms,
                                    ...
                                    price_min=price_min, price_max=price_max,
                                    pool_key=pk, confirmed=confirmed, need=need, attempts=attempts, on_status=on_status)
